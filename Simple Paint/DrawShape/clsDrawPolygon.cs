...
namespace Simple_Paint
{
    ...
    public class clsDrawPolygon : DrawShape
    {
        ...
        public override void Draw(Graphics gp)
        {
            ...
            {
                gpPath.AddLine(lstPoints[0],lstPoints[1]);
                gp.DrawLine(pen, lstPoints[0], lstPoints[1]);
            }
            else if (lstPoints.Count > 2)
            {
                // Vẽ Curve
                gpPath.AddPolygon(lstPoints.ToArray());
                gp.DrawPolygon(pen, lstPoints.ToArray());
            }
            if (isSelected)
            {
                lstPoints.ForEach(p =>
                ...
                using (var pen = new Pen(Color.White, (float)1.2))
                    if (lstPoints.Count == 2 && lstPoints[0] != lstPoints[lstPoints.Count - 1])
                    {
                        gp.DrawLine(pen, lstPoints[0], lstPoints[1]);
                    }
